...
<body>  <div id="jsonViewer" style="display: none; background: var(--card-color); padding: 1rem; margin: 2rem; border-radius: 8px;">
    ...
    <h1 class="site-title" onclick="navigate('home')">منصة شمس التعليمية</h1>
    ...
    <div class="video-page">
      ...
      <img src="https://www2.0zz0.com/2025/05/06/18/178585837.jpg" alt="قناة التليجرام">
      <h3>اشترك في قناة التليجرام</h3><p>تابع آخر التحديثات والمحتوى الجديد</p>
      <a href="https://t.me/M203SM" target="_blank">اذهب للقناة</a></div></div>  <script src="https://cdn.jsdelivr.net/npm/hls.js@latest"></script>  <script src="https://cdn.plyr.io/3.7.2/plyr.polyfilled.js"></script>  <script type="text/babel">
    ...
    function playVideo(ci,li,url,title,desc){currentClassIndex=ci;currentLectureIndex=li;currentLectureLink=url;var video=document.getElementById('player');if(url.endsWith('.m3u8')){if(hls){hls.destroy();}if(Hls.isSupported()){hls=new Hls();hls.loadSource("proxy.php?url=" + encodeURIComponent(url));;hls.attachMedia(video);hls.on(Hls.Events.MANIFEST_PARSED,function(){player.play();});}else if(video.canPlayType('application/vnd.apple.mpegurl')){video.src=url;video.addEventListener('loadedmetadata',function(){player.play();});}else{alert('هذا المتصفح لا يدعم بث الفيديو بهذا الصيغة');return;}}else{player.source={type:'video',sources:[{src:url,type:'video/mp4'}]};player.play();}document.getElementById('videoTitle').textContent=title;document.getElementById('videoDescription').textContent=desc;navigate('video');}
    ...
    function loadData(){var errorMsg='<p class="error">حدث خطأ في تحميل البيانات</p>';
      fetch('data.json',{headers:{'Referer':'https://careful-burnt-honesty.glitch.me/'}})
      .then(function(res){if(!res.ok)throw new Error('فشل تحميل البيانات');return res.json();})
      .then(function(data){teachers=data.teachers;renderTeachers();console.log('بيانات JSON:',data);})
      .catch(function(error){console.error('Error:',error);teachersGrid.innerHTML=errorMsg;teachersGrid.innerHTML+='\n<button onclick="location.reload()" style="margin-top:1rem;padding:0.5rem 1rem;background:var(--primary);color:#fff;border:none;border-radius:5px;">إعادة تحميل البيانات</button>';});}
